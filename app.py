...
def page_style():
    image = Image.open('WEB_APP/avia_model.jpeg')
    icon_image = Image.open('WEB_APP/icon_avia.jpg')
    ...
    st.title('Предсказание удовлетворенности клиента авиакомпании полетом')
...
def prediction_user(user_input):
    with open('WEB_APP/model.pickle', 'rb') as f_1:
        model = pickle.load(f_1)
    with open('WEB_APP/ohe.pickle', 'rb') as f_2:
        ohe = pickle.load(f_2)
    with open('WEB_APP/scaler.pickle', 'rb') as f_3:
...
def importsnce_plot():
    with open('WEB_APP/model.pickle', 'rb') as f:
        model = pickle.load(f)
